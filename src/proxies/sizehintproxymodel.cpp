...
public:
    ...
    ContextAdapterFactory        *m_pContextAdapterFactory   {nullptr};
    ContextAdapter        *m_pContextAdapter   {nullptr};
...
void SizeHintProxyModelPrivate::reloadContext(QAbstractItemModel *m)
{
    ...
    if (m_pContextAdapterFactory)
        delete m_pContextAdapterFactory;

    m_pContextAdapterFactory = new ContextAdapterFactory();
    m_pContextAdapterFactory->setModel(m);

    QQmlContext* qmlCtx = QQmlEngine::contextForObject(q_ptr);
    Q_ASSERT(qmlCtx && qmlCtx->engine());

    m_pContextAdapter = m_pContextAdapterFactory->createAdapter(qmlCtx);

    m_pContextAdapter->setCacheEnabled(false);
}
...
void SizeHintProxyModelPrivate::reloadContants()
{
    ...
    if (!m_pQmlContext) {
        QQmlContext* qmlCtx = QQmlEngine::contextForObject(q_ptr);
        Q_ASSERT(qmlCtx && qmlCtx->engine());
        m_pQmlContext = m_pContextAdapter->context();
...
qreal SizeHintProxyModelPrivate::evaluateForIndex(QQmlExpression* expr, const QModelIndex& idx)
{
    ...
    m_pContextAdapter->setModelIndex(idx);
